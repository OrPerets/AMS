...
const skeletonVariants = cva('animate-pulse rounded-md bg-muted', {
  variants: {
    variant: {
      ...
      morph: 'bg-gradient-to-br from-muted via-muted/90 to-muted-foreground/12 animate-[shimmer_2.4s_ease-in-out_infinite]',
    },
  },
  defaultVariants: {
    variant: 'default',
...
const SkeletonText = ({ lines = 3, className }: { lines?: number; className?: string }) => (
  <div className={cn('space-y-2', className)}>
    {Array.from({ length: lines }).map((_, i) => (
      <Skeleton key={i} className={cn('h-4', i === lines - 1 ? 'w-3/4' : 'w-full')} variant="shimmer" />
    ))}
  </div>
);
...
const SkeletonTable = ({ rows = 5, columns = 4 }: { rows?: number; columns?: number }) => (
  <div className="space-y-3">
    ...
    {Array.from({ length: rows }).map((_, rowIndex) => (
      <div key={`row-${rowIndex}`} className="grid grid-cols-4 gap-4 border-b p-4">
        {Array.from({ length: columns }).map((_, colIndex) => (
          <Skeleton key={`cell-${rowIndex}-${colIndex}`} className={cn('h-4', colIndex === 0 ? 'w-full' : 'w-3/4')} />
        ))}
      </div>
    ))}
  </div>
...
const SkeletonAvatar = ({ size = 'md' }: { size?: 'sm' | 'md' | 'lg' }) => {
  ...
  return <Skeleton className={cn('rounded-full', sizeClasses[size])} variant="shimmer" />;
};
