...
function Stage({ children, index }: { children: React.ReactNode; index: number }) {
  return (
    <motion.div
      layout
...
export function DashboardPageSkeleton() {
  return (
    <motion.div className="space-y-5" initial={false}>
      <Stage index={0}>
        <Skeleton className="h-52 rounded-[30px]" variant="morph" />
      </Stage>
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {Array.from({ length: 4 }).map((_, index) => (
          <Stage key={index} index={index + 1}>
            <Skeleton className="h-32 rounded-[26px]" variant="morph" />
          </Stage>
        ))}
      </div>
      <div className="grid gap-5 xl:grid-cols-[1.05fr_0.95fr]">
        {[0, 1].map((column) => (
          <Stage key={column} index={column + 5}>
            <Skeleton className="h-[360px] rounded-[30px]" variant="morph" />
          </Stage>
        ))}
      </div>
    </motion.div>
    ...
    <Card variant="elevated" className="overflow-hidden">
      <CardContent className="space-y-4 p-4 sm:p-6">
        ...
        <div className="overflow-hidden rounded-[24px] border border-subtle-border bg-card/80">
          <div className="grid gap-4 border-b border-subtle-border bg-muted/35 px-4 py-4" style={{ gridTemplateColumns: `repeat(${columns}, minmax(0, 1fr))` }}>
          ...
          <div className="space-y-0">
            {Array.from({ length: rows }).map((_, rowIndex) => (
              <div key={`row-${rowIndex}`} className="grid gap-4 border-b border-subtle-border/70 px-4 py-4 last:border-b-0" style={{ gridTemplateColumns: `repeat(${columns}, minmax(0, 1fr))` }}>
                {Array.from({ length: columns }).map((_, colIndex) => (
                  <Skeleton key={`cell-${rowIndex}-${colIndex}`} className={colIndex === 0 ? 'h-4 w-4/5 rounded-full' : 'h-4 w-3/5 rounded-full'} variant="shimmer" />
                ))}
              </div>
            ))}
          </div>
...
export function DetailPanelSkeleton() {
  return (
    <motion.div className="space-y-4" initial={false}>
      <Stage index={0}>
        <Skeleton className="h-36 rounded-[28px]" variant="morph" />
      </Stage>
      <Stage index={1}>
        <Card variant="elevated">
          <CardContent className="space-y-5 p-5 sm:p-6">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <Skeleton className="h-28 rounded-[22px]" variant="morph" />
              <Skeleton className="h-28 rounded-[22px]" variant="morph" />
            </div>
            <Skeleton className="h-44 rounded-[24px]" variant="morph" />
          </CardContent>
        </Card>
      </Stage>
    </motion.div>
...
export function MobileCardSkeleton({ cards = 3 }: { cards?: number }) {
  return (
    <motion.div className="space-y-4" initial={false}>
      <Stage index={0}>
        <Skeleton className="h-40 rounded-[28px]" variant="morph" />
      </Stage>
            ...
            <Card variant="elevated" className="overflow-hidden">
              <CardContent className="space-y-4 p-5">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <Skeleton className="h-11 w-11 rounded-[18px]" variant="morph" />
                </div>
                <Skeleton className="h-4 w-full rounded-full" variant="shimmer" />
                <Skeleton className="h-4 w-5/6 rounded-full" variant="shimmer" />
                <motion.div layout className="rounded-[22px] border border-subtle-border/70 bg-muted/24 p-4">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  <Skeleton className="h-11 rounded-full" variant="morph" />
                  <Skeleton className="h-11 rounded-full" variant="morph" />
                </div>
              </CardContent>
            </Card>
