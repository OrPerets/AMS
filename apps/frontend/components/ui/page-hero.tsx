...
export function PageHero({
  ...
  if (compact) {
    return (
      ...
      >
        ...
        <div className="relative flex flex-col gap-4 p-4 sm:flex-row sm:items-end sm:justify-between sm:gap-6 sm:p-5 lg:p-6">
          <div className="min-w-0 space-y-2">
            ...
            <div className="space-y-1.5">
              <h1 className="max-w-[18ch] text-[1.2rem] font-bold leading-[1.05] text-white sm:text-[1.45rem] lg:text-[1.7rem]">{title}</h1>
              {description ? <p className="max-w-2xl text-sm leading-6 text-white/76">{description}</p> : null}
            </div>
          </div>
  ...
  return (
    ...
    >
      <div className="pointer-events-none absolute inset-0">
        <div className="absolute start-[-12%] top-[-16%] h-48 w-48 rounded-full bg-white/8 blur-3xl" />
        <div className="absolute end-[-5%] top-[18%] h-44 w-44 rounded-full bg-primary/18 blur-3xl" />
        <div className="absolute inset-x-0 bottom-0 h-24 bg-linear-to-t from-black/16 to-transparent" />
      </div>

      <div className="relative grid gap-4 p-4 sm:gap-5 sm:p-5 lg:grid-cols-[minmax(0,1.05fr)_minmax(16rem,0.95fr)] lg:gap-8 lg:p-7">
        <div className="space-y-4 sm:space-y-5">
          <div className="space-y-2.5 sm:space-y-3">
            ...
            <div className="space-y-2">
              ...
              <h1 className="max-w-[15ch] text-[1.7rem] font-bold leading-[0.98] tracking-[-0.03em] text-white sm:text-[2.35rem] lg:text-[3rem]">{title}</h1>
              {description ? <div className="max-w-2xl text-[0.95rem] leading-7 text-white/76">{description}</div> : null}
            </div>
          </div>
