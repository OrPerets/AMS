...
test.describe('sprint 10 tier-1 visual smoke', () => {
  ...
  test('captures authenticated tier-1 pages across resident and manager journeys', async ({ page }, testInfo) => {
    const pageChecks = [
      { path: '/resident/account', role: 'RESIDENT' as const, readyHeading: 'האזור האישי של הדייר', screenshot: 'tier1-resident-account.png' },
      { path: '/resident/requests', role: 'RESIDENT' as const, readyHeading: 'בקשות דייר', screenshot: 'tier1-resident-requests.png' },
      ...
      { path: '/admin/dashboard', role: 'PM' as const, readyHeading: 'לוח בקרה ניהולי', screenshot: 'tier1-admin-dashboard.png' },
